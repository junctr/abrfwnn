...
#     phi1 = np.empty((3,1), dtype=np.float64)
    
#     phi1 = qd[:,[1]] + e[:,[1]] + ((6 * np.identity(3, dtype=np.float64)) @ e[:,[0]])
    
#     return phi1

# def z2_f(q, phi1):
    
#     z2 = np.empty((3,1), dtype=np.float64)
...
def taud_f(e, s, taus0, taus1, y):
    ...
    taud = np.empty((3,1), dtype=np.float64)
...
def vk_f(mu, muji, A, Aold, B, a):
    ...
    vk = np.empty((5,75), dtype=np.float64)
...
def ak_f(mu, muji, A, Aold, B ,v, a, b, xold):
    ...
    ak = np.empty((5,75), dtype=np.float64)
...
for n_seed in range(5):
    
    np.random.seed(n_seed)
    print(n_seed)

    alpha_w = 50 * np.identity(5, dtype=np.float64)
    alpha_v = 40 * np.identity(75, dtype=np.float64)
    alpha_a = 40 * np.identity(75, dtype=np.float64)
    alpha_b = 40 * np.identity(75, dtype=np.float64)
    ...
    param_all = [v,a,b,W,beta,zeta]
    #param_all_old = [vold,aold,bold,Wold,betaold,zetaold]
    ...
    np.save(f"abrfwnn/data_test_seed/p_s{n_seed}_m{alpha_lambda}_wn{alpha_wn0}_{alpha_wn1}_s{alpha_s0}_{alpha_s1}_{alpha_s2}_T{T}_step{step}_t{end}_param_all.npy",param_all)
    #np.save(f"k_s{n_seed}_m{alpha_lambda}_T{T}_t{end}_param_all_old.npy",param_all_old)
    np.savetxt(f"abrfwnn/data_test_seed/p_s{n_seed}_m{alpha_lambda}_wn{alpha_wn0}_{alpha_wn1}_s{alpha_s0}_{alpha_s1}_{alpha_s2}_T{T}_step{step}_t{end}_e_all.csv",e_all)
